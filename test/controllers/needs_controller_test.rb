...
class NeedsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create need" do
    assert_difference('Need.count') do
      post needs_url, params: { need: { amount: @need.amount, name: @need.name, priority: @need.priority, status: @need.status } }
    end
  ...
  test "should update need" do
    patch need_url(@need), params: { need: { amount: @need.amount, name: @need.name, priority: @need.priority, status: @need.status } }
